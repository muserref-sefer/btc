...
function NewsDetailItem({ news }: { news: INews }) {
  ...
  function isNewsInReadList(newsTitle: string): boolean {
    ...
    const readList: string[] = JSON.parse(localStorage.getItem("readList") || "[]");
    return readList.includes(newsSlug);
  }

  function renderToggleRead() {
    if (isExistNews) {
      return <><img src="/assets/icon-remove.svg" alt="plus"/> Remove from read list</>;
    } else {
      return <><img src="/assets/icon-plus.svg" alt="plus"/> Add to read list</>;
    }
  }

  function toggleReadList() {
    const newsSlug = slugify(news.title);
    let readList: string[] = JSON.parse(localStorage.getItem("readList") || "[]");
  ...
  return (
    <>
      ...
      <LazyLoadImage className="newsImage" src={news.urlToImage || notFoundImage} alt="news-img" onError={onImageError} placeholderSrc={placeholderImage} />
      <p className="content">{news.content}</p>
      <div className="linkWrapper">
        <button className="link" onClick={toggleReadList}>
          {renderToggleRead()}
        </button>
        <span className="date">{moment(news.publishedAt).utc().format('DD.MM')}</span>
      </div>
    </>
  )
}
