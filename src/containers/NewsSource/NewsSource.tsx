...
function NewsSource() {
  ...
  const fetchNews = () => {
    ...
    return fetch(`https://newsapi.org/v2/top-headlines/?sources=${slug}&language=en&apiKey=${process.env.REACT_APP_API_KEY}`)
      .then((res) => {
        if (!res.ok) {
      ...
      .then((data) => {
        if(data.status === "ok") {
          const sortedData = data.articles.sort((prev: { publishedAt: moment.MomentInput; }, next: { publishedAt: moment.MomentInput; }) => {
        ...
        <div data-testid="newsSourceContainer">
          ...
          <div className="newsListContainer">
            <div className="newsListHeader">
              ...
              <Link className="newsBtn" to="/">{'<'} Go to News</Link>
            </div>
            <NewsList />
          </div>
        </div>
