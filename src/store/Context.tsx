...
const NewsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const filterNewsSourcesByCategory = (categories: string[]) => {
    ...
    const filteredItems = newsSources.filter(newsSource => categories.includes(newsSource.category))
  ...
  return (
    <NewsContext.Provider value={{ newsSources, setNewsSources, filteredNewsSources, filterNewsSourcesByCategory, news, setNews }}>
      {children}
